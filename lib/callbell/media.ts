/**
 * Media de Callbell — lógica PURA (sin I/O, sin `server-only`), testeable directo.
 *
 * Los `attachments` del webhook son URLs. Estos helpers infieren el content-type,
 * el nombre de archivo y el tipo de media, y arman el data URL base64. La descarga
 * real (que usa el secreto de Callbell) vive en `mediaFetch.ts` (server-only).
